...
public class Orthodrome extends Controller {
    ...
    public Orthodrome(SphericalTriangle sphericalTriangle, Point aPoint, Point bPoint, Controller.CASE caseType) {
    ...
    public Point calculateFirstOrthodromeVertex() {
        double numerator =
                Math.tan(getaPoint().getPhiRadians()) * Math.sin(getbPoint().getLambdaRadians())
                        - Math.tan(getbPoint().getPhiRadians()) * Math.sin(getaPoint().getLambdaRadians());

        double denominator =
                Math.tan(getaPoint().getPhiRadians()) * Math.cos(getbPoint().getLambdaRadians())
                        - Math.tan(getbPoint().getPhiRadians()) * Math.cos(getaPoint().getLambdaRadians());
        ...
        if (!getCaseType().equals(Controller.CASE.GENERAL)) {
    ...
    private void verifySpecialCasesHeight() {
        if (getCaseType() == Controller.CASE.EQUATOR_SAIL) {
            this.setHeight1(90);
            this.setHeight2(999);
    ...
    private Point verifySpecialCasesFirstOrthodromeVertex() {
        ...
        } else if (getCaseType() == Controller.CASE.MERIDIAN_SAIL) {
